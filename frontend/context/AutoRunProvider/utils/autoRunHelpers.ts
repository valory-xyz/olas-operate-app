 ...
 *
 * Trade-off: `lastStartedAtRef` is ephemeral per Pearl session (plain useRef,
 * not persisted). After a Pearl restart every alternate has `lastStartedAt = 0`
 * so even a genuinely-earned alternate gets overridden to `false` on the first
 * poll after restart. The scanner may then try to start an already-earned
 * agent — which is a no-op on-chain (one wasted start cycle, no data loss).
 * This is intentional: Pearl cannot distinguish "genuinely earned this epoch"
...
export const refreshRewardsEligibility = async ({
  ...
  if (!response) {
    ...
    // `sleepAwareDelay` returns false only on sleep/wake drift — not on
    // AutoRun being disabled. Bail so the retry doesn't run against a
    // potentially-stale view of on-chain state after a suspend/resume.
    if (!waitOk) return;
    try {
      response = await fetchAgentStakingRewardsInfo({
        chainId: meta.chainId,
