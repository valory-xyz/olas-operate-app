...
type AddBackupWalletPasswordModalProps = {
  open: boolean;
  onClose: () => void;
};

export const AddBackupWalletPasswordModal = ({
  open,
  onClose,
}: AddBackupWalletPasswordModalProps) => {
  const { goto } = useSettings();
  const { setPassword } = useAddBackupWallet();
  ...
  return (
    <Modal
      ...
      title="Authorize Backup Wallet Setup"
      action={
