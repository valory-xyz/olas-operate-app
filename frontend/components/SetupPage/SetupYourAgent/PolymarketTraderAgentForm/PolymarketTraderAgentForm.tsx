...
import {
  PolymarketTraderFieldValues,
  usePolymarketTraderFormValidate,
} from './usePolymarketTraderFormValidate';

type PolymarketTraderAgentFormContentProps = {
  serviceTemplate: ServiceTemplate;
};

/**
 * Form for setting up a Polymarket Trader agent (To setup and update the agent).
 */
export const PolymarketTraderAgentFormContent = ({
  serviceTemplate,
}: PolymarketTraderAgentFormContentProps) => {
  const [form] = Form.useForm<PolymarketTraderFieldValues>();
  const { goto } = useSetup();
  ...
  const {
    ...
    validateForm,
  } = usePolymarketTraderFormValidate('Finish Agent Configuration');

  const onFinish = useCallback(
    async (values: PolymarketTraderFieldValues) => {
  ...
  return (
    <Form<PolymarketTraderFieldValues>
      form={form}
...
type PolymarketTraderAgentSetupFormProps = { serviceTemplate: ServiceTemplate };

export const PolymarketTraderAgentSetup = ({
  serviceTemplate,
  renderForm,
}: Pick<PolymarketTraderAgentSetupFormProps, 'serviceTemplate'> & {
  renderForm: RenderForm;
}) => {
  return renderForm(
    <PolymarketTraderAgentFormContent serviceTemplate={serviceTemplate} />,
    <>
      <GeminiApiKeyDesc />
    </>,
    { onBack: undefined },
