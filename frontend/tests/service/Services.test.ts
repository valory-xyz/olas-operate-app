...
describe('ServicesService', () => {
  ...
  describe('createService', () => {
    ...
    it('sets use_staking:false per chain config when stakingProgramId is no_staking', async () => {
      const serviceTemplate = makeServiceTemplate({
        configurations: {
          [MiddlewareChainMap.GNOSIS]: {
            nft: 'bafybeig64atqaladigoc3ds4arltdu63wkdrk3gesjfvnfdmz35amv7faq',
      ...
      await ServicesService.createService({
        deploy: true,
        serviceTemplate,
        stakingProgramId: 'no_staking' as StakingProgramId,
      });
      ...
      for (const [middlewareChain, config] of Object.entries(
        ...
        expect((config as Record<string, unknown>).use_staking).toBe(false);
        // sanity: rpc override still applied
        expect((config as Record<string, unknown>).rpc).toBe(
          CHAIN_CONFIG[asEvmChainId(middlewareChain)].rpc,
        );
      }
    });

    it('does not set use_staking for a normal staking program', async () => {
      const serviceTemplate = makeServiceTemplate();
      jest
        .spyOn(global, 'fetch')
        .mockReturnValue(mockJsonResponse(MOCK_SERVICE_RESPONSE));
      ...
      expect(gnosisConfig.staking_program_id).toBe(stakingProgramId);
      expect('use_staking' in gnosisConfig).toBe(false);
    });
