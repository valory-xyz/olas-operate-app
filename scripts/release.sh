# THIS SECTION OF THE CODE HANDLES THE PROCESS OF RELEASING.
# NOTE: you have to be on the correct branch to run this script and release.

# Usage: ./release.sh <rc-number>
# Example 1: ./release.sh 3
#  - This will create a release branch (locally), update version numbers, commit, and tag the release as 0.3.0-rc3.
# Example 2: ./release.sh 200-signed
#  - This will create a release branch (locally), update version numbers, commit, and tag the release as 0.3.0-rc200-signed.
#  - NOTE: The package.json and pyproject.toml files will be updated as "0.3.0-rc200".
...
RC_VALUE=$1
VERSION="0.3.0-rc$RC_VALUE"
BRANCH="release/030-rc$RC_VALUE"
...
VERSION_FIELD="0.3.0-rc$RC_NUMBER"
...
if [ -f pyproject.toml ]; then
  ...
  sed -i '' -E "s/^version = \".*\"/version = \"$VERSION_FIELD\"/" pyproject.toml
else
...
# Step 7: Delete existing tag if it exists locally and remotely
