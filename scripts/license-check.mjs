...
// Bound the subprocess so a stuck node_modules walk can't pin the CI
// job at the workflow-level timeout. 5 min is well above the typical
// 30s scan time for ~1000-pkg trees.
...
function runChecker() {
  return new Promise((resolvePromise) => {
    // `shell: true` is required on Windows so the `npx.cmd` shim in
    // PATH resolves; harmless on Linux/macOS. Pinned version matches
    // the devDependency in package.json — `npx` will use the local
 ...
 * - String with " OR " → SPDX OR (any allowed → allowed).
 * - String with " AND " → SPDX AND (all must be allowed; any unauthorized → unauthorized).
 * - Plain string → look up directly (after normalization).
 */
function evalExpr(raw, allowedSet, unauthorizedSet) {
  ...
  // Strip a single outer paren pair before splitting; nested expressions
  // are unusual in npm and would need a real parser if they show up.
...
for (const [pkgVersion, info] of Object.entries(report)) {
  ...
  // Exemption short-circuits everything: if a package is exempted, the
  // license is irrelevant. (Use sparingly; copyleft only.)
  ...
  // Override: replace the declared license with the resolved SPDX, then
  // evaluate normally. Catches UNKNOWNs whose LICENSE file is permissive
  // (e.g. map-stream) without silencing them.
...
// Drift detection: overrides/exemptions whose package is no longer in the tree.
const stale = [];
...
if (overridden.length) {
  ...
  for (const { name, pkgVersion, declared, entry } of overridden) {
