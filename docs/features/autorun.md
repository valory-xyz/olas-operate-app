...
| `runningAgentTypeRef` | Currently running agent type (from react-query polling) |
| `runningServiceConfigIdRef` | Currently running instance's serviceConfigId |
| `lastRewardsEligibilityRef` | Per-instance previous rewards state; guards duplicate rotation triggers |
...
| `stopRetryBackoffUntilRef` | Per-instance exponential backoff after stop timeout |
...
| `AGENT_SELECTION_WAIT_TIMEOUT_SECONDS` | 60s | Hard timeout for UI selection to match requested instance |
| `REWARDS_WAIT_TIMEOUT_SECONDS` | 20s | Hard timeout for rewards snapshot to arrive |
| `DISABLE_RACE_STOP_CHECK_INTERVAL_MS` | 10s | Poll cadence when checking for an instance that started during a concurrent disable |
...
4. `rotateToNext(currentServiceConfigId)`:
   a. Refresh rewards for all other instances.
...
3. Evicted and past `minimumStakingDuration` → `stopAgentWithRecovery`, then `COOLDOWN_SECONDS`, then `startAgentWithRetries` for the same instance; the start re-stakes it through the middleware's deploy endpoint. A failed stop aborts before the start and records the usual backoff. The cooldown is the same one rotation uses — the backend needs time to tear the service down before a start on the same instance is accepted. `runningSinceRef` is reset afterwards, since the instance id does not change across the recovery, as is the rewards guard.
...
The running instance is re-checked after the read as well as before it: a rotation can *complete* while the staking call is in flight, and acting on the original id then would stop a different agent.

**Why the callbacks are held in refs.** `getDeployabilityForRunningInstance`, `startAgentWithRetries` and `rotateToNext` are all kept in refs rather than listed as effect dependencies. They close over data derived from the services query, which refetches every 5 s; left in the dependency array they would tear down and recreate this interval faster than its 10-minute period, so the check could never fire. The runtime watchdog survives the same churn only because `runningSinceRef` carries its progress across restarts — this check is stateless, so it has to avoid the churn instead. `useAutoRunLifecycle.test.ts` pins this with a re-render test.
...
- **Fix**: `lastRewardsEligibilityRef.current[currentServiceConfigId] = undefined` added immediately after a successful stop in `rotateToNext`, before the cooldown delay. Mirrors the reset already present on stop failure and in both catch blocks.

### Code — `waitForEligibilityReady` extracted to shared utility
- Identical 60s-timeout polling loop existed in both `useAutoRunScanner` and `useAutoRunStartOperations`. Extracted to `waitForEligibilityReadyHelper` in `utils/autoRunHelpers.ts`; both hooks delegate to it via `useCallback`.
