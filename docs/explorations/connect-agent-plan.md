...
PR 1 — Everything up to "Select agent" (creation DISABLED)
  ...
  SelectChain screen: chain select (ordered, one-per-chain disable) + funding display +
    "Select agent" button rendered DISABLED. AgentOnboarding routes Connect → SelectChain.
  → /review-implementation

PR 2 — Enable creation + skip staking + funding methods + intro
  Enable "Select agent" → connect-gated create (single-chain template clone,
